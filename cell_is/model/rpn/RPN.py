# -*- coding: utf-8 -*-
"""
Created on Thu Dec  2 15:48:16 2021
...
from model.utils import _smooth_l1_loss
...
class RPN(nn.Module):
    ...
    def __init__(self, din):
        """
            din: depth of the image after backbone
        """
        self.din = din
        self.anchor_ratio = cfg.ANCHOR_RATIOS
        self.anchor_scale = cfg.ANCHOR_SCALES
        self.stride = cfg.STRIDE[0]
        
        # define the convrelu layers processing input feature map
        self.RPN_Conv = nn.Conv2d(self.din, 512, 3, 1, 1, bias=True) # Conv-layer: 3*3
        
        # define bg/fg classifcation score layer
        self.nc_score_out = len(self.anchor_scales) * len(self.anchor_ratios) * 2 # 2(bg/fg) * 9 (anchors)
        # self.nc_score_out = 1 * len(self.anchor_ratios) * 2 # Here take attention
        self.RPN_cls_score = nn.Conv2d(512, self.nc_score_out, 1, 1, 0)
        
        # define anchor box offset prediction layer
        self.nc_bbox_out = len(self.anchor_scales) * len(self.anchor_ratios) * 4 # 4(coords) * 9 (anchors)
        # self.nc_bbox_out = 1 * len(self.anchor_ratios) * 4 # Here take attention
        self.RPN_bbox_pred = nn.Conv2d(512, self.nc_bbox_out, 1, 1, 0)#就是下面那条线的1*1
        ...
        def forward(self, rpn_feature_maps, im_info, gt_boxes, num_boxes):
            ...
            for i in range(n_feat_maps):
                ...
                rpn_shapes.append([rpn_cls_score.size()[2], rpn_cls_score.size()[3]])#[H, w]
